...
model=Model(input_img,predictions, name='inception_resnet')
model.load_weights("/home/zenin/weights100v4.19-1.44.hdf5")
# Compile the model
...
tensorboardcall=callbacks.TensorBoard(log_dir='/home/zenin/logs100v4',write_graph=True, write_images=True,
                                      embeddings_freq=50, embeddings_layer_names=None, embeddings_metadata=None)
checkpointer = callbacks.ModelCheckpoint(filepath="/home/zenin/weights100v4.{epoch:02d}-{val_loss:.2f}.hdf5",monitor='val_loss', verbose=1, save_best_only=True, mode='auto', period=10)
...
model_info = model.fit_generator(datagen.flow(train_features, train_labels, batch_size = 40),
                                 ...
                                 validation_data = (test_features, test_labels),initial_epoch=20, callbacks=[tensorboardcall,checkpointer,lrate])
end = time.time()
