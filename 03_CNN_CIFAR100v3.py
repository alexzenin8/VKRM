...
x = conv2d_bn(input_img, 32, 3, 3, strides=(1, 1), padding='same')
...
def step_decay(epoch):
    ...
    return lrate
# Compile the model
rms = RMSprop(lr=0.0,epsilon=1.0)
model.load_weights("/home/zenin/weights100v3.09-2.24.hdf5")
...
tensorboardcall=callbacks.TensorBoard(log_dir='/home/zenin/logs100v3',write_graph=True, write_images=True,
                                      embeddings_freq=50, embeddings_layer_names=None, embeddings_metadata=None)
checkpointer = callbacks.ModelCheckpoint(filepath="/home/zenin/weights100v3.{epoch:02d}-{val_loss:.2f}.hdf5",monitor='val_loss', verbose=1, save_best_only=True, mode='auto', period=10)
lrate = callbacks.LearningRateScheduler(step_decay)
# train the model
start = time.time()
# Train the model
model_info = model.fit_generator(datagen.flow(train_features, train_labels, batch_size = 100),
                                 samples_per_epoch = train_features.shape[0], epochs = 200,
                                 validation_data = (test_features, test_labels),initial_epoch=9, callbacks=[tensorboardcall,checkpointer,lrate])
end = time.time()
